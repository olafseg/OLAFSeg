...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        #detach img and copy to variable img_obj
        img_obj = img.copy()
...
model = DeepLabFactored(backbone='resnet101', num_anim_classes=9, num_inanim_classes=15)

obj_model.load_state_dict(torch.load('/home2/pranavgupta77/obj.pth')['state_dict'])
model.load_state_dict(torch.load('/ssd_scratch/cvit/pranav.g/float/ckpt/58_combined_v2_104.pth'))
...
def combine_obj_part_pred(objs, parts, animate):
    ...
    preds_bg = (preds == 0).astype(int)
    objs_nonbg = (objs > 0).astype(int)
    conflicts = (objs_nonbg * preds_bg)
    return preds, np.sum(conflicts)

def combine_obj_all_parts(objs, anim_parts, inanim_parts):
    anim_final, anim_conflicts = combine_obj_part_pred(objs, anim_parts, True)
    inanim_final, inanim_conflicts = combine_obj_part_pred(objs, inanim_parts, False)
...
output_dir = '/home2/pranavgupta77/codes/58/wacv/outputs/58_combined_v2/'
