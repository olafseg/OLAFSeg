...
ckpt_path = '/ssd_scratch/cvit/pranav.g/float/ckpt/'
gpu_ids = [0,1,2,3]
...
class DeepLabFactored(nn.Module):
    def __init__(self, num_anim_classes, num_inanim_classes, backbone='resnet101', output_stride=16,
        ...
        self.lrbf_aspp_low = build_aspp('drn', output_stride, BatchNorm)
    ...
    def forward(self, input):
        ...
        lrfb_x = self.lrfb_aspp(x4)
        lrbf_low_aspp = self.lrbf_aspp_low(features)
        lr = self.lr_decoder(lrfb_x, low_level_feat, lrbf_low_aspp)
        lr = F.interpolate(lr, size=input.size()[2:], mode='bilinear', align_corners=True)

        fb = self.fb_decoder(lrfb_x, low_level_feat, lrbf_low_aspp)
    ...
    def get_10x_lr_params(self):
        modules = [self.features, self.anim_aspp, self.inanim_aspp, self.lrfb_aspp, self.anim_aspp_low, self.inanim_aspp_low, self.lrbf_aspp_low, self.anim_decoder,
...
def objmask_loss(pred, macro_gt, num_classes, weighted=True,
    ...
    pred = F.interpolate(pred, size=macro_gt.shape[1:], mode='bilinear', align_corners=True)
...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        part_label_path = self.folder + 'GT_part_201/' + self.image_path_list[i] + '.png'
        obj_label_path = self.folder + 'object/' + self.image_path_list[i] + '.png'
        fb_label_path = self.folder + 'fb_from_obj_201/' + self.image_path_list[i] + '.png'
        edge_label_path = self.folder + 'hed_edges_201_2/' + self.image_path_list[i] + '.png'
...
for epoch in range(num_epochs):
    ...
    adj1_miogt_avg = mIOUMeter(2)
    adj2_miogt_avg = mIOUMeter(2)
    ...
    for i, sample in enumerate(tbar):
        ...
        for cl in range(2):
            ious[cl] = ious[cl].item()
            counts[cl] = counts[cl].item()
        adj1_miogt_avg.update(ious, counts)

        ious, counts = iogt_perpart(pred_adj2, adj2_o, 3)
        for cl in range(2):
            ious[cl] = ious[cl].item()
            counts[cl] = counts[cl].item()
        adj2_miogt_avg.update(ious, counts)
        ...
        tbar.set_description('a:%.4f, i:%.4f, lr:%.3f, fb:%.3f' % (anim_alliou_avg.Mean_Intersection_over_Union(),
    ...
    print('Left    : {:.5f}'.format(adj1_miogt_avg.vals[0]/adj1_miogt_avg.counts[0]))
    print('Right   : {:.5f}'.format(adj1_miogt_avg.vals[1]/adj1_miogt_avg.counts[1]))
    print('LR      : {:.5f}'.format(adj1_miogt_avg.avg))

    print('Front   : {:.5f}'.format(adj2_miogt_avg.vals[0]/adj2_miogt_avg.counts[0]))
    print('Back    : {:.5f}'.format(adj2_miogt_avg.vals[1]/adj2_miogt_avg.counts[1]))
    print('FB      : {:.5f}'.format(adj2_miogt_avg.avg))
    print("Epoch time taken:", str(epoch_end-epoch_start))
    print('----------------------------------------')

    torch.save(model.module.state_dict(), ckpt_path + '201_combined_v2_' + str(epoch) + '.pth')
